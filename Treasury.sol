...
contract TimeTreasury is Ownable {
    ...
    /**
        @notice allow approved address to deposit an asset for RUG
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint RUG needed and store amount of rewards for distribution
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( RUGGED ).balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with RUG
        @param _amount uint
     */
    function repayDebtWithRUG( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        if ( _managing == MANAGING.RESERVEDEPOSITOR ) { // 0
            ...
            sRUGQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
